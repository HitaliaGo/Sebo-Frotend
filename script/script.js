...
// A função 'fetchAndDisplayBooks' é responsável por buscar a lista de livros da API e exibir esses livros na página.
//Faz uma requisição 'GET' PARA 'Http:localhost:5029/api/livro/'
//Limpa o conteúdo atual do container de cartões 
//Para cada livro recebido,cria um cartão com informações do livro e adiciona ao container.
//Adiciona um evento de clique no botão de detalhes para redicionar para a página de detalhes do livro.
    ...
    try {
        ...
        livros.forEach(livro => {
            ...
            card.innerHTML = `
                ...
                <h4>${livro.titulo}</h4>
                 <h3>Preço: R$ ${livro.preco.toFixed(2)}</h3>
...
//'getGeneroString' :Coverte um valor númerico de gênero em uma string correspondente
...
//'getEnumValue' :Converte uma string de gênero em seu valor númerico correspondente.
...
//A função 'loadBookDetails' busca e exibe osa detalhes de um livro específico.
...
//Adiciona um evento de clique ao botão de edição para redirecionar para página de edição do livro.
...
//A função 'loadEditForm' carrega os dados de um livro em um formulário de edição.
//Obtém o IB do livro a partir da URL.
//Faz uma requisição 'GET' para obter os detalhes do livro.
//Adiciona um evento de envio ao formulário para atualizar o livro requisição 'PATCH'
...
//A função 'cadastrar livros' cadastra um novo livro na base 
//Prevê o comportamento padrão do formulário.
//Coleta os dados do formulári e cria um objeto livro.
...
//Chamam 'fetchAndDisplayBooks' para carregar os livros.
//Adicioonam eventos aos formulários e botões conforme a página atual('index.html''detalhes.html','editar.html')
...
//A função 'fetchAndDisplayBooks' é reutilizada para atualizar a lista de livros após cadastro ou exclusão.
//A função 'loadBookDetails' é chamada duas vezes, o que é redundante.Recomendo refatorar para evitar duplicação.
//A função 'loadEditForm' lida com a edição  do livro,coletando os dados atuais e enviando uma atualização.
//A função 'cadastrarLivro' lida com o cadastro de novos livros.
//O modal de confirmação de exclusão é exibido quando o botão de deletar é clicado e oculta quando o cancelamento é escolhido.
